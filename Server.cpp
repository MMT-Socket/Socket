...
enum class SendMsgFlag : unsigned char
{
    REGISTER_FAIL = 0,
    REGISTER_SUCCESS,
    LOGIN_FAIL_USERNAME,
...
void Write_User_Info_To_Database(vector<User> Database) {
    ...
    if (out.is_open())
    {
        ...
        for (int i = 0; i < Database.size(); i++)
        {
            ...
            cout << Database[i].Account << "-" << Database[i].Password << "-" << Database[i].Fullname << "-" << Database[i].Date << "-" << Database[i].Point << "." << endl;
...
void Login(SOCKET client,vector<User>& Database,User& user) {
...
int process_client(client_type& new_client, std::vector<client_type>& client_array, std::thread& thread)
{
    ...
    while (1)
    {
        ...
        case 1: //Login
        {
            Login(new_client.socket,Database,user);
            break;
        }
        case 2: // REGISTER
        {
        ...
        case 100://  LOGOUT  : Shut down client
        {
            ...
            /*ShutdownClient(new_client.socket, client_array, new_client.id);*/
